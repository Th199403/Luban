...
import Business, {
    ...
    RequestPhotoInfo,
    ToolHeadType,
} from "../../lib/SACP-SDK/SACP/business/Business";
...
import CoordinateInfo, {
    Direction,
} from "../../lib/SACP-SDK/SACP/business/models/CoordinateInfo";
import MovementInstruction, {
    MoveDirection,
} from "../../lib/SACP-SDK/SACP/business/models/MovementInstruction";
...
class SocketTCP {
    ...
    public connectionOpen = (socket: SocketServer, options: EventOptions) => {
        ...
        this.sacpClient.setHandler(0x01, 0x03, (data) => {
            ...
            this.sacpClient.getModuleInfo().then(({ data: moduleInfos }) => {
                const info = moduleInfos.find(
                    (moduleInfo) => moduleInfo.moduleId === 14
                );
                this.sacpClient
                    .getLaserToolHeadInfo(info.key)
                    .then(({ laserToolHeadInfo }) => {
                        this.laserFocalLength =
                            laserToolHeadInfo.laserFocalLength;
                        log.debug(
                            `laserToolHeadInfo.laserFocalLength, ${laserToolHeadInfo.laserFocalLength}`
                        );
                        this.socket &&
        ...
        this.client.connect(
            ...
            () => {
                ...
                const result = {
                    ...
                    text:
                        '{"token":"415344a6-f7b7-4900-a391-aa6695e2dfdb","readonly":false,"series":"Snapmaker A400","headType":2,"hasEnclosure":false}',
                };
    ...
    public startHeartbeat = () => {
        this.sacpClient
            .subscribeHeartbeat({ interval: 1000 }, () => {
    ...
    public getPosition = async (
        ...
        return this.sacpClient
            .getPosition(params)
            .then(({ response, x, y, z }) => {
                console.log("positionX", x, y, z);
                if (response.result === 0) {
                    callback({ status: true });
                } else {
                    callback({ status: false });
    ...
    public setPosition = async (
        params: any,
        callback: (result: { status: boolean }) => void
    ) => {
        return this.sacpClient.setPosition(params).then(({ response }) => {
            if (response.result === 0) {
                callback({ status: true });
            } else {
                callback({ status: false });
    ...
    public getCameraCalibration = (
        callback: (matrix: CalibrationInfo) => void
    ) => {
        return this.sacpClient
            .getCameraCalibration(ToolHeadType.LASER10000mW)
            .then(({ response }) => {
                if (response.result === 0) {
                    const calibrationInfo = new CalibrationInfo().fromBuffer(
                        response.data
                    );
    ...
    public getPhoto = (
        callback: (result: { success: boolean; filename: string }) => void
    ) => {
    ...
    public getCalibrationPhoto = (
        callback: (result: { success: boolean; filename: string }) => void
    ) => {
        return this.sacpClient
            .getCalibrationPhoto(ToolHeadType.LASER10000mW)
            .then(({ response, data }) => {
                let success = false;
                let filename = "";
                if (response.result === 0) {
    ...
    public setMatrix = (
        params: { matrix: CalibrationInfo },
        callback: (result: string) => void
    ) => {
        return this.sacpClient
            .setMatrix(ToolHeadType.LASER10000mW, params.matrix)
            .then(({ response }) => {
                if (response.result === 0) {
                    callback("");
                } else {
    ...
    public getLaserMaterialThickness = (options: EventOptions) => {
        ...
        this.sacpClient
            ...
            .then(async ({ response, thickness }) => {
                ...
                try {
                    const res1 = await this.sacpClient.updateCoordinate(
                        CoordinateType.MACHINE
                    );
                    log.debug(`=====, ${res1}`);
                    this.sacpClient
                        .getCurrentCoordinateInfo()
                        .then(async ({ coordinateSystemInfo }) => {
                            const xNow = coordinateSystemInfo.coordinates.find(
                                (item) => item.key === Direction.X1
                            ).value;
                            const yNow = coordinateSystemInfo.coordinates.find(
                                (item) => item.key === Direction.Y1
                            ).value;
                            const zNow = coordinateSystemInfo.coordinates.find(
                                (item) => item.key === Direction.Z1
                            ).value;

                            log.debug(
                                `current positions, ${xNow}, ${yNow}, ${zNow}`
                            );
                            log.debug(
                                `thickness & focal, ${this.thickness}, ${this.laserFocalLength}`
                            );

                            await this.sacpClient.updateCoordinate(
                                CoordinateType.WORKSPACE
                            );

                            const newX = new CoordinateInfo(Direction.X1, xNow);
                            const newY = new CoordinateInfo(Direction.Y1, yNow);
                            const newZ = new CoordinateInfo(
                                Direction.Z1,
                                zNow - (this.laserFocalLength + this.thickness)
                            );
                            ...
                            const zMove = new MovementInstruction(
                                MoveDirection.Z1,
                                0
                            );
    ...
    public executeGcode = async (
        ...
        try {
            callback && callback();
            this.socket &&
                this.socket.emit("connection:executeGcode", {
                    msg: "",
                    res: null,
                });
        } catch (e) {
    ...
    public uploadGcodeFile = (
        gcodeFilePath: string,
        type: string,
        callback: (msg: string, data: boolean) => void
    ) => {
