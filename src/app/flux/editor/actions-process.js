...
export const processActions = {
    recalculateAllToolPath: (headType) => (dispatch, getState) => {
        ...
        toolPathGroup.toolPaths.forEach((toolPath) => {
            ...
            toolPathPromiseArray.push(
                toolPathGroup.commitToolPathPromise(toolPath?.id, { materials })
            );
        });
    ...
    preview: (headType) => (dispatch, getState) => {
        const { SVGActions, toolPathGroup, progressStatesManager } = getState()[
            headType
        ];
    ...
    showToolPathGroupObject: (headType) => (dispatch, getState) => {
        const { modelGroup, toolPathGroup, displayedType } = getState()[
            headType
        ];
        if (displayedType === DISPLAYED_TYPE_TOOLPATH) {
            return;
    ...
    selectModelInProcess: (headType, intersect, selectEvent) => (
        dispatch,
        getState
    ) => {
        ...
        if (!intersect) {
            ...
            if (selectEvent === SELECTEVENT.UNSELECT_ADDSELECT) {
                ...
                const text1 = i18n._(
                    'key-CncLaser/Toast-Failed to generate a toolpath. Selected objects should be of the same type.'
                );
                if (getToolPathType([...selectedModels, model]).length !== 1) {
                    if (!toastId || !toast.isActive(toastId)) {
                        ...
                        toastId = toast(
                            ToastWapper(text1, 'WarningTipsTips', '#1890ff')
                        );
                    }
                } else if (
                    selectedModels.findIndex((m) => m === model) === -1
                ) {
    ...
    updateToolPath: (headType, toolPathId, newState) => (
        dispatch,
        getState
    ) => {
    ...
    deleteToolPath: (headType, selectedToolPathIDArray) => (
        dispatch,
        getState
    ) => {
        const { toolPathGroup, displayedType, modelGroup } = getState()[
            headType
        ];
    ...
    commitGenerateToolPath: (headType, toolPathId) => (dispatch, getState) => {
        const { toolPathGroup, materials, progressStatesManager } = getState()[
            headType
        ];
    ...
    commitGenerateGcode: (headType) => (dispatch, getState) => {
        ...
        dispatch(
            baseActions.updateState(headType, { isGcodeGenerating: true })
        );
        dispatch(
            baseActions.updateState(headType, {
        ...
        console.log('commitGenerateGcode2', new Date(), headType, toolPaths);
    ...
    onGenerateGcode: (headType, taskResult) => async (dispatch, getState) => {
        const { modelGroup, toolPathGroup, progressStatesManager } = getState()[
            headType
        ];
        ...
        const renderGcodeFileName = `${_.replace(
            currentModelName,
            /(\.svg|\.dxf|\.png|\.jpg|\.jpeg|\.bmp)$/,
            ''
        )}_${new Date().getTime()}${suffix}`;
    ...
    commitGenerateViewPath: (headType) => (dispatch, getState) => {
        ...
        progressStatesManager.startProgress(PROCESS_STAGE.CNC_LASER_VIEW_PATH, [
            1,
            1,
        ]);
        dispatch(
        ...
        const viewPathInfos = toolPathGroup.getCommitGenerateViewPathInfos({
            materials,
        });
    ...
    onGenerateViewPath: (headType, taskResult) => async (
        dispatch,
        getState
    ) => {
        ...
        await toolPathGroup.onGenerateViewPath(
            viewPathFile,
            isRotate ? materials : boundarySize
        );
    ...
    _checkModelsInChunkArea: (headType) => (dispatch, getState) => {
        ...
        if (materials.isRotate) {
            ...
            const text1 = i18n._(
                'key-CncLaser/Toast-Moving objects to this area may cause a machine collision.'
            );
            if (svgSelectedGroupBoundingBox.y < ChunkAreaY) {
                if (!toastId || !toast.isActive(toastId)) {
                    toastId = toast(
                        ToastWapper(text1, 'WarningTipsWarning', '#FFA940')
                    );
                }
            }
        }
    ...
    // profile manager
    changeActiveToolListDefinition: (
        headType,
        definitionId,
        name,
        ...
        const activeToolListDefinition = toolDefinitions.find(
    ...
    duplicateToolListDefinition: (headType, activeToolListDefinition) => async (
        dispatch,
        getState
    ) => {
