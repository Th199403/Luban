...
function scaleToFitWithRotate(data) {
    return new Observable(observer => {
        try {
            ...
            const { positionAttribute, normalAttribute, matrixWorlds, left, right, front, back, selectedGroupMatrix, selectedCount } = data;
            let { size } = data;
            let offsetX = 0;
            let maxScale = -1;
            let rotateAngel = 0;
            ...
            positionAttribute.send.forEach((positionItem, index) => {
                const tempGeometry = new THREE.BufferGeometry();
                tempGeometry.setAttribute('position', new THREE.BufferAttribute(positionItem.array, positionItem.itemSize));
                tempGeometry.setAttribute('normal', new THREE.BufferAttribute(normalAttribute.send[index].array, normalAttribute.send[index].itemSize));
